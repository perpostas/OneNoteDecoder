...
#include <cstddef>
#include <cstdint>
#include "FileNodeBase.hpp"

class ObjectSpaceManifestListReferenceFND : public FileNodeBase,
        FileNodeChunkReference(this.stpFormat, this.cbFormat) {
    private:
        uint32_t stpFormat;
        uint32_t cbFormat;
    
    public:
        FileNodeChunkReference refField;
        ExtendedGuid gosid;


        ObjectSpaceManifestListReferenceFND(uint32_t stpFmt, uint32_t cbFmt) {
            stpFormat = stpFmt;
            cbFormat = cbFmt;
        }

        size_t DoDeserializeFromByteArray(char *pStart)
        {
            char *pIndex = pStart;
            pIndex += refField.Deserialize(pIndex);
            pIndex += gosid.Deserialize(pIndex);
