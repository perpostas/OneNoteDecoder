...
class ExtendedGuid {
    public:
        ...
        size_t Deserialize(char *begin) {
            ...
            byteIndex += guid.Deserialize(byteIndex);
            byteIndex += Primitive::Deserialize(N, byteIndex);

            // Return a length of ExtendedGuid structure
            return static_cast<size_t>(byteIndex - begin);
        }

        bool CompareTo(ExtendedGuid &extendedGuid) {
            return false;
        }
};